...
function buildWatchList(wearableData: any): WatchEntry[] {
  ...
  return items
    .map((item: any) => {
      ...
      if (item.isLocked && item.minLevel) lockParts.push(`Level ${item.minLevel}`);
      if (item.isPrestigeLocked && item.prestigeRequired) lockParts.push(`Prestige ${item.prestigeRequired}`);
      return {
        key: slug,
        id: item.id,
        label: item.name,
        ...
        lockReason: lockParts.length > 0 ? `Requires ${lockParts.join(" & ")}` : null,
      };
    })
  ...
  return carData.catalog
    .map((car: any) => {
      ...
      return {
        ...
        lockReason: car.lockReason ?? (car.isLocked ? `Reach level ${car.minLevel}` : null),
      };
    })
    .filter(Boolean) as CarEntry[];
}
...
export default function CustomizeScreen() {
  ...
  useEffect(() => {
    const ownedWatches = watches.filter((w) => w.isOwned);
    const ownedCars = cars.filter((c) => c.isOwned);
    const preloadFiles = [
      ...ownedWatches.slice(0, 3).map((w) => w.glbFile).filter(Boolean),
      ...ownedCars.slice(0, 3).map((c) => c.glbFile).filter(Boolean),
    ];
  ...
  const selectWatch = useCallback(
    (w: WatchEntry) => {
      ...
      if (w.isLocked) {
        Alert.alert("Locked", w.lockReason ?? "This watch is locked.");
        return;
      }
      if (!w.isOwned) {
        Alert.alert("Not Owned", "Purchase this watch from the Wardrobe first.", [
          { text: "Go to Wardrobe", onPress: () => router.push("/wearables" as any) },
  ...
  const selectCar = useCallback(
    (c: CarEntry) => {
      Haptics.selectionAsync().catch(() => {});
      if (c.isLocked) {
        Alert.alert("Locked", c.lockReason ?? "This vehicle is locked.");
        return;
      }
      if (!c.isOwned) {
        Alert.alert("Not Owned", "Purchase this vehicle from the Garage first.", [
          { text: "Go to Garage", onPress: () => router.push("/cars" as any) },
          { text: "Cancel", style: "cancel" },
        ]);
        return;
      }
                ...
                <View style={s.grid}>
                  {watches.map((w) => (
                    <TouchableOpacity
                      ...
                      style={[
                        s.card,
                        selectedWatch?.key === w.key && w.isOwned && s.cardOn,
                        (w.isLocked || !w.isOwned) && s.cardLocked,
                      ]}
                    ...
                    >
                      ...
                      {!w.isLocked && !w.isOwned && (
                        <View style={s.lockBadge}>
                          <Ionicons name="cart-outline" size={10} color="#FFF" />
                        </View>
                      )}
                      {selectedWatch?.key === w.key && w.isOwned && (
                        <Text style={s.checkBadge}>✓</Text>
                      )}
                      <Text style={[s.cardIcon, (w.isLocked || !w.isOwned) && { opacity: 0.4 }]}>{w.icon}</Text>
                      <Text style={[s.cardLabel, (w.isLocked || !w.isOwned) && { opacity: 0.5 }]} numberOfLines={2}>
                        {w.label}
                      </Text>
                      {w.isLocked && w.lockReason && (
                        <Text style={s.lockLabel}>{w.lockReason}</Text>
                      )}
                    </TouchableOpacity>
                  ))}
                </View>
                  ...
                  {cars.map((c) => (
                    <TouchableOpacity
                      ...
                      style={[
                        s.card,
                        s.carCard,
                        selectedCar?.key === c.key && c.isOwned && s.cardOn,
                        (c.isLocked || !c.isOwned) && s.cardLocked,
                      ]}
                      onPress={() => selectCar(c)}
                      activeOpacity={0.75}
                    >
                      ...
                      {!c.isLocked && !c.isOwned && (
                        <View style={s.lockBadge}>
                          <Ionicons name="cart-outline" size={10} color="#FFF" />
                        </View>
                      )}
                      {selectedCar?.key === c.key && c.isOwned && (
                        <Text style={s.checkBadge}>✓</Text>
                      )}
                      <Text style={[s.cardIcon, (c.isLocked || !c.isOwned) && { opacity: 0.4 }]}>{c.icon}</Text>
                      <Text style={[s.cardLabel, (c.isLocked || !c.isOwned) && { opacity: 0.5 }]} numberOfLines={2}>
                        {c.label}
                      </Text>
                      {c.isLocked && c.lockReason && (
                        <Text style={s.lockLabel}>{c.lockReason}</Text>
                      )}
                      {c.prestige > 0 && !c.isLocked && (
...
const s = StyleSheet.create({
  ...
  cardLocked: { borderColor: "#333", opacity: 0.7 },
