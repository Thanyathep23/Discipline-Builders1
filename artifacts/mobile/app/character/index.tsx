...
export type WearableWatch = { id: string; slug: string; name: string; watchStyle: "basic" | "refined" | "elite"; styleEffect: string | null; glbFile?: string | null } | null;
...
export default function CharacterStatusScreen() {
  ...
  // Prefer watch GLB from server API response; fall back to AsyncStorage for backward compat
  useEffect(() => {
    const apiWatchGlb = equippedW?.watch?.glbFile ?? null;
    if (apiWatchGlb) {
      setEquippedWatchGlb(apiWatchGlb);
    } else {
      getEquippedWatch().then(w => {
        setEquippedWatchGlb(w?.glbFile ?? null);
      });
    }
  }, [equippedW?.watch?.glbFile]);
