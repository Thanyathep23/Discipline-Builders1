...
function NativeOutfitModelInner({
  ...
  useEffect(() => {
    ...
    const bodyScale = bodyRef.current
  ...
  useFrame(() => {
    if (!alignedRef.current && gltf?.scene && bodyRef.current) {
      const s = gltf.scene;
      s.scale.setScalar(bodyRef.current.scale.x * align.scale);
      const scaledBox = new THREE.Box3().setFromObject(s);
      const scaledCenter = new THREE.Vector3();
      scaledBox.getCenter(scaledCenter);
      s.position.set(
...
function WebModelViewer({
  ...
  const [stableUrl] = useState(
    () =>
      `${API_BASE}/character-viewer.html?skin=${encodeURIComponent(skinTexture)}&hair=${encodeURIComponent(hairModel)}&hairTex=${encodeURIComponent(hairTexture)}&gender=${encodeURIComponent(gender)}${outfitGlb ? `&outfit=${encodeURIComponent(outfitGlb)}` : ""}${watchGlb ? `&watch=${encodeURIComponent(watchGlb)}` : ""}`
  );
  ...
  }, [hairModel, skinTexture, hairTexture, gender, outfitGlb, watchGlb, sendMessage]);
