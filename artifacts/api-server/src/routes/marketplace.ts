...
router.post("/:itemId/buy", requireAuth, async (req: any, res) => {
  try {
    ...
    console.log(`[buy] user=${userId} item=${item.id} (${item.name}) cost=${item.cost} balance: ${user.coinBalance} → ${newBalance}${devMode ? " [devMode]" : ""}`);
    ...
    console.log(`[buy] ✓ committed — user=${userId} item=${item.id} newBalance=${newBalance}`);
